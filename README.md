...
### 🌐 메타크리틱 데이터 수집 매커니즘 (`zelda_crawler.py`)
본 프로젝트에 사용된 글로벌 게이머 유저 리뷰 데이터셋은 메타크리틱 웹 서버가 통신 시 사용하는 내부 백엔드 JSON API 엔드포인트를 정밀하게 역공학(Reverse Engineering)하여 수집하였습니다. 일반적인 웹 스크래핑 시 발생하는 차단 문제를 해결하고 안정적인 대량 수집 파이프라인을 구축하기 위해 다음과 같은 고급 크롤링 알고리즘 아키텍처를 반영하였습니다.

1. **엔드포인트 추적 및 인증**: 메타크리틱의 웹 프론트엔드가 비동기 통신을 시도하는 내부 API 주소(`https://backend.metacritic.com/reviews/metacritic/user/games/.../web`)와 클라이언트 발급 전용 `apiKey`를 획득하여 직접 쿼리를 전송했습니다.
2. **봇 차단 우회 메커니즘 (Anti-Bot Bypass)**: 메타크리틱 방화벽 시스템의 자동 웹 크롤러 차단(HTTP 403 Forbidden 및 IP 차단)을 방어하기 위해, 파이썬 `requests` 라이브러리 송신 헤더에 브라우저의 실제 환경값인 고유 `User-Agent` 문자열을 커스텀 주입하여 일반 사용자의 유기적 트래픽으로 완벽히 위장하였습니다.
3. **페이지네이션 및 속도 제한 조율 (Rate Limiting)**: API의 `offset`과 `limit` 파라미터를 동적으로 변화시키는 `while` 루프 구조를 설계하여 1회 호출당 100건씩 페이지 단위로 수집 프로세스를 무한 동력 가동하였으며, 서버 과부하 및 패킷 드랍을 방지하기 위해 요청 주기 사이에 정밀한 딜레이 타임아웃을 연동하였습니다.
